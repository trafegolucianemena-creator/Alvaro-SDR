(function () {
  const funnelytics = {
        ...
        (n && clearTimeout(n),
          (funnelytics.projects._inputListeners[e.target] = setTimeout(function () {
            ...
            (n.open('POST', funnelytics.origin + '/set-attributes'),
              ...
              n.send(
                'd=' +
                  encodeURIComponent(JSON.stringify({ session: funnelytics.session, project: funnelytics.project, attributes: { email: e.target.value } })),
              ),
    ...
    cookies: {
      ...
      expire: function (cookieName) {
        ...
        document.cookie = cookieName + '=' + ';path=/; SameSite=None; Secure;' + (domain ? ';domain=' + domain : '') + ';expires=Thu, 01 Jan 1970 00:00:01 GMT';
      },
    },
    url: {
      regex: new RegExp(/.*:\/\/.*\..*/),
        ...
        if (!funnelytics.client.isBot())
          if (funnelytics.session) {
            ...
            (n.open('POST', funnelytics.origin + '/steps'),
              ...
              n.send(
                'd=' + encodeURIComponent(JSON.stringify({ project: funnelytics.project, session: funnelytics.session, page: window.location.href, referrer })),
              ));
  ...
  function setupVimeoVideos() {
    ...
    vimeoIframes.forEach((iframe) => {
      ...
      player.getVideoTitle().then((title) => {
        ...
        // Track initial mute state so we can detect changes
        let wasMuted = null;
        ...
        player.on('volumechange', function (data) {
          ...
          if (data.muted !== wasMuted) {
            if (!data.muted) {
              trackVideoEvent('Vimeo', 'unmute', title);
            } else {
              trackVideoEvent('Vimeo', 'mute', title);
            }
            wasMuted = data.muted;
          }
        });
  ...
  function loadWistiaTracking() {
    const hasModern = document.querySelector('wistia-player');
    // iframe with wistia_embed class = legacy path, even if it has web_component=true param
    const hasLegacy = document.querySelector('iframe[src*="wistia.net"]') || document.querySelector('.wistia_embed');

    if (hasModern) setupModernWistiaTracking();
    if (hasLegacy) setupLegacyWistiaTracking();
  }

  // Modern <wistia-player> web component
  function setupModernWistiaTracking() {
    document.querySelectorAll('wistia-player').forEach(function (player) {
      function bindEvents() {
        const videoTitle = player.name || player.getAttribute('media-id') || 'Wistia Video';
        ...
        // time-update is the correct event name for the Aurora web component
        player.addEventListener('time-update', function (e) {
          const currentTime = e.detail.currentTime;
          const duration = e.detail.duration;
          if (duration) trackPercent(currentTime, duration);
        });
      }

      // 'api-ready' is the correct event per Wistia Aurora docs
      player.addEventListener('api-ready', function () {
        bindEvents();
      });

      // If api-ready already fired before this script ran
  ...
  // Legacy iframe / .wistia_embed (including iframe?web_component=true)
  function setupLegacyWistiaTracking() {
    function pushLegacyQueue() {
    ...
    // E-v1.js is the required bridge script for iframe embeds
    if (typeof Wistia === 'undefined') {
      loadScript('https://fast.wistia.net/assets/external/E-v1.js', pushLegacyQueue);
    } else {
      pushLegacyQueue();
    }
  }
  ...
  function detectAndInitVideoTracking() {
    ...
    if (document.querySelector('iframe[src*="wistia.net"]') || document.querySelector('.wistia_embed') || document.querySelector('wistia-player'))
      loadWistiaTracking();
